...
def annotation(body, label="", background="#ddd", color="#333", type='replace', **style):
    ...
    **style : dict
    ...
    return span(
        ...
        )(span(_class='tooltiptext')(label))
    )
    # if type == 'replace' else \
    # span(
    #     style=styles(
    #         background=background,
    #         border_radius=rem(0.33),
    ...
    *args : str, tuple or htbuilder.HtmlElement
    ...
    for arg in args:
        if isinstance(arg, str):
            ...
            out(annotation(*arg, type=arg[-1]))

        else:
            raise Exception("Oh noes!")
    streamlit.components.v1.html(css + str(out), **kwargs, scrolling=True, height=max(250, len(args) * 1.8))